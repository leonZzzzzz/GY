...
Page({
  ...
  saveAddress(e) {
    ...
    var fullname = that.data.fullname;//姓名
    var phonename = that.data.phonename;//电话
    var address = that.data.address;//具体地址
    var uid = that.data.uid;//用户id
    var pro_id = that.data.pro_id;//省id
    var cit_id = that.data.cit_id;//市id
    var ar_id = that.data.ar_id;//区id
    if (!this.checkAddress()) { return; }
    if (fullname && phonename && address && uid && pro_id && cit_id) {
      var params = {
        user_name: fullname,
        tel: phonename,
        province: pro_id,
